...
fact_store(action="add",
    content="ヒロイン: 桜井美咲。年齢19歳。身長158cm。黒髪ロングストレート。大学生。性格は真面目で思いやりがあるが、内面は好奇心旺盛。一人称「私」。口調は丁寧語。趣味は読書と星空観察。",
    category="character", tags="character,ヒロイン,美咲")

fact_store(action="add",
    content="世界観: 舞台は近未来の東京。2045年、第三次世界大戦後の復興都市。AIが社会インフラを管理し、人間は創造的労働に特化。",
...
```python
# セマンティック検索（自然文でOK）
fact_store(action="search", query="翔太のトラウマについて")

# エンティティ全件取得
fact_store(action="probe", entity="桜井美咲")

# 関連エンティティ探索
fact_store(action="related", entity="桜井美咲")

# 複数エンティティ横断
fact_store(action="reason", entities=["桜井美咲", "佐藤翔太"])
...
| `search` | **日本語セマンティック検索** | 「翔太の過去」を自然文で検索 |
| `probe` | エンティティ全件取得 | キャラの全情報を一括確認 |
| `related` | エンティティ関連探索 | 関連キャラ・情報を横断検索 |
